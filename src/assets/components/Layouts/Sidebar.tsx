...
const Sidebar = () => {
  ...
  return (
    <aside
      className={`${sidebarOpen ? "col-span-2" : "col-span-0 items-center w-20"} transition-all duration-300 flex flex-col justify-between px-2 py-[10px] h-[calc(100vh-73px)]`}>
        ...
        <li className="h-fit w-fit mb-[-6px]">
          <button onClick={toggleSidebar} className={`px-[3px] py-[5px] text-white w-full flex items-center justify-start rounded hover:bg-gray-200 active:bg-gray-400 focus:outline-none ${sidebarOpen ? 'bg-white' : 'bg-gray-300'}`}>
            <div className="space-y-[4px]">
              <div className="w-[30px] h-[4px] bg-gray-500 rounded"></div>
              <div className="w-[30px] h-[4px] bg-gray-500 rounded"></div>
              <div className="w-[30px] h-[4px] bg-gray-500 rounded"></div>
            </div>
          </button>
